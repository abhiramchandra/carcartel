...
def inquiry(request):
    if request.method == "POST":
        ...
        last_name=request.POST['lats-name']
        ...
        if request.user.is_authenticated:
            ...
            has_contacted=Contact.objects.all().filter(car_id, user_id=user_id)
            if has_contacted:
                message.error(request, 'You have already made an inquiry about this car. Please wait untill we get back to you.')
